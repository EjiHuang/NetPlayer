/*
 https://github.com/sipsorcery-org/SIPSorceryMedia.FFmpeg/blob/master/src/FFmpegVideoEncoder.cs
 */
...
namespace NetPlayer.WinUI.FFmpeg.Core.Encoder
{
    public sealed unsafe class FFmpegVideoEncoder : IVideoEncoder, IDisposable
    {
        ...
        private object _encoderLock = new();
        private object _decoderLock = new();
        ...
        private AVPixelFormat GetAVPixelFormat(VideoPixelFormatsEnum pixelFormat)
        {
            return pixelFormat switch
            {
                ...
                VideoPixelFormatsEnum.I420 => AVPixelFormat.AV_PIX_FMT_YUV420P,
                VideoPixelFormatsEnum.NV12 => AVPixelFormat.AV_PIX_FMT_NV12,
                VideoPixelFormatsEnum.Rgb => AVPixelFormat.AV_PIX_FMT_RGB24,
        ...
        private void InitialiseDecoder(AVCodecID codecID)
        {
            if (!_isDecoderInitialised)
            {
                ...
                logger.Debug($"[InitialiseDecoder] CodecId:[{codecID}");
            }
        }
            ...
            if (!_isDisposed)
            {
                lock (_encoderLock)
                {
                    ...
                    try
                    {
                        ...
                        {
                            logger.Debug("Video encoder needs more data.");
                            return null;
                        }
                        else
                        {
        ...
        public void Dispose()
        {
            logger.Debug("VideoEncoder dispose.");
