        ...
        private void DoWorkerJob(System.Net.Sockets.UdpClient socket, int data_port)
        {
            ...
            try
            {
                ...
                while (true)
                {
                    ...
                    NetPlayer.Core.RTSP.Messages.RtspChunk currentMessage = new NetPlayer.Core.RTSP.Messages.RtspData();
                    // aMessage.SourcePort = ??
                    currentMessage.Data = frame;
                    ((NetPlayer.Core.RTSP.Messages.RtspData)currentMessage).Channel = data_port;


                    OnDataReceived(new NetPlayer.Core.RTSP.RtspChunkEventArgs(currentMessage));
